...
import Document, { AnyPageProps, Main, NextScript, PageProps } from 'next/document';
  ...
  public render() {
    ...
    const theme =
      pageContext && typeof pageContext.theme === 'function'
        ...
        : undefined;
    // @ts-ignore
    const themeColor = theme ? theme.palette.primary.main : 'white';

    return (
      <html lang={i18n.language} dir="ltr">
        <GlobalHead lng={i18n.language} themeColor={themeColor} />
        <body>
          <Main />
          <NextScript />
        </body>
...
CustomDocument.getInitialProps = (ctx) => {
  ...
  const page = ctx.renderPage((Component: ComponentType<IPagePropsWithPageContext>) => {
    const WrappedComponent: ComponentType<{ pageContext: IPageContext } & PageProps> = (
      props,
    ) => {
